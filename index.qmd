...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      ...
      numericInput("plat_x", "Coordenadas de plataforma en X", value = 117.8),  # Default value set to 220
      numericInput("plat_y", "Coordenadas de plataforma en Y", value = 38.4),   # Default value set to 77
...
1. Correr la función.
2. Especificar el directorio del archivo a analizar. El archivo tiene que tener las siguientes características: formato `CVS`; 3 columnas (tiempo [en segundos], X [en pixeles], Y [en pixeles]). NOTA, se pude hacer con las coordenadas en centimetros, pulgadas...etc, pero se tiene que ajustar la función en caso de no proveer en pixeles. [Ver aquí ejemplo de archivo](https://github.com/santi-rios/maestria_app_water_maze/blob/main/ejemplo_datos.csv).
